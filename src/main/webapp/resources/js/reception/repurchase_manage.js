			...
			.dataTable(
					{
						...
						"aoColumns" : [
								...
								},
								{
									...
									"render" : function(data, type, full) {
										var result = "";
										var checkButton = "<button class='btn btn-block btn-info' id='cId"
												+ data
												+ "' onclick='checkOrder("
												+ data + ");'>确认 </button>";
 ...
 */
/*
 * function checkOrder(orderId){ $.ajax({ "dataType": 'json', "type": "POST",
 * "url": base+"repurchase/check", "data": { "orderId":orderId }, "success":
 * function(data){ alert("商品回购已经通知库房，请到库房进行回购业务");
 * $('#orderTable').dataTable().fnDraw(); }, error : function(XMLHttpRequest,
 * textStatus, errorThrown) { alert("error"); } }) }
 */

/**
 * 审批通过页面：显示名称，数量，修改单价回购 输入金额，点击确认
 * 
 * @param orderId
 * @returns
 */
