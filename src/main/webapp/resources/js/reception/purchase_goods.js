			...
			.dataTable(
					{
						...
						"columnDefs" : [ {
							...
							"render" : function(data, type, full) {
								return "<button class='btn btn-info' id='addId"
										+ data
										+ "' onclick='addGoods("
										+ data
										+ ");'> 加入购物车</button>";
			...
			.ajax({
				...
				success : function(data) {
					...
					$
							.each(
									data.aaData,
									function(index, goodsForm) {
										item += "<tr><td>"
												+ parseInt(index + 1) + "</td>";
										item += "<td>" + goodsForm.code
												+ "</td>";
										item += "<td>" + goodsForm.name
												+ "</td>";
										item += "<td>" + goodsForm.price
												+ "</td>";
										item += "<td>" + goodsForm.unit
												+ "</td>";
										item += "<td>" + goodsForm.amount
												+ "</td>";
										item += "<td><label class='checkbox-inline' style='padding-top:0px;margin-right:0px'><input type='checkbox' name='row_checkbox'  id='check"
												+ goodsForm.id
...
function addOtherGoods() {
	...
	if ($("input[name=row_checkbox]:checked").length > 0) {
		$("input[name=row_checkbox]:checked")
				.each(
						function() {
							var id = $(this).attr("id");
							addGoodsDiv(id.substr(5, id.length), $(this)
									.parent().parent().prev().prev().prev()
									.prev().html(), $(this).parent().parent()
									.prev().prev().prev().html());
						});
...
function gotoAccount() {
	...
	// alert(goodInfoJson)
	window.location.href = base + "/order/account?goodInfo="
			+ encodeURI(goodInfoJson);
}

// 添加购物车
function addGoods(id) {
	addGoodsDiv(id, $("#addId" + id).parent().prev().prev().prev().prev()
			.prev().html(), $("#addId" + id).parent().prev().prev().prev()
			.prev().html());
}

// 修改页面内容
function addGoodsDiv(id, name, price) {
	var item = "<tr id=tr" + id + "><td name='goodName'>商品名称：" + name
			+ " </td><td>商品售价：<em name='goodPrice'>" + price + " </em>￥ </td>";
	...
	item += " <span class='label label-danger' style='float: right' onclick='delGoods("
			+ id + ");'>删除</span>";
	item += "</td></tr>";
