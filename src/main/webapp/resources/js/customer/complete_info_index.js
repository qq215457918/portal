$(document)
		.ready(
				function() {
					...
					if ($('#importResult').val() != '') {
						alert("部分数据导入失败，第" + $('#importResult').val()
								+ "行出现问题，请检查数据");
					}
					...
					$('#exportCustomer')
							.click(
									function() {
										...
										exportHtml += '<input type="hidden" name="payPriceS" value="'
												+ payPriceS + '"/>'
										exportHtml += '<input type="hidden" name="payPriceE" value="'
												+ payPriceE + '"/>'
										exportHtml += '<input type="hidden" name="createDateS" value="'
												+ dpd1 + '"/>'
										exportHtml += '<input type="hidden" name="createDateE" value="'
												+ dpd2 + '"/>'
										exportHtml += '<input type="hidden" name="area" value="'
												+ area + '"/>'
							...
							.click(
									function() {
										...
										if (flag == 1) {
											$('#importExcelForm')
													.attr('action',
															'customerInfo/importCustomerAdd');
										} else if (flag == 0) {
											$('#importExcelForm')
													.attr('action',
											...
											$('#importExcelForm')
													.attr('action',
															'customerInfo/updateCustomerPhoneStaff');
										}
									});
				});
...
$(document)
		.on(
				...
				function() {
					...
					window.location.href = "customerInfo/customerOrderInfoIndex?customerId="
							+ customerId
				});
			...
			.dataTable(
					{
						...
						"columnDefs" : [ {
							"render" : function(data, type, row) {
								return '<a href="javascript:void(0);" data-customer-id="'
										+ row.id + '" id="orderDetail">详情</a>';
							},
							"targets" : 8
						} ],
