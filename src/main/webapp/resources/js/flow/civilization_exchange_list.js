			...
			$('#goodsDetailList button')
					.click(
							function() {
								...
								$('#goodsDetailList input[name=cultureFlag]')
										.val(flag);
								if (confirm(content)) {
									$('#updateCivi').submit();
								}
...
$(document).on(
		...
		'#orderDetailInfo',
		function() {
			$('#goodsDetailList input[name=orderId]').val(
			...
			$.ajax({
				...
				"success" : function(data) {
					...
					for ( var i in data) {
						if (j == 0) {
							$('#goodsDetailList #cultureRemark').val(
									data[i].remark);
							j++;
						}
			...
			.dataTable(
					{
						...
						"columnDefs" : [ {
							"render" : function(data, type, row) {
								var operation = '<a class="btn btn-info btn-block" href="#goodsDetailList" data-toggle="modal" data-order-id="'
										+ row.id
										+ '" id="orderDetailInfo">详情</a>';
								return operation;
							},
							"targets" : 10
						} ],
